...
class AcademicSources:
    ...
    async def _openalex(self, query: str, limit: int) -> list[SpecialistResult]:
        ...
        for item in body.get("results", [])[:limit]:
            ...
            for authorship in (item.get("authorships") or [])[:8]:
                ...
                if author.get("display_name"): authors.append(str(author["display_name"]))
            out.append(SpecialistResult(
                url=url, title=str(item.get("display_name") or item.get("title") or ""),
                snippet=("Authors: " + ", ".join(authors)) if authors else "", engine="openalex", category="academic",
                published_at=str(item.get("publication_date") or "") or None,
                raw={"openalex_id": item.get("id"), "doi": doi, "cited_by_count": item.get("cited_by_count"), "type": item.get("type")},
            ))
        return out
    ...
    async def _crossref(self, query: str, limit: int) -> list[SpecialistResult]:
        ...
        async with httpx.AsyncClient(timeout=self.timeout, trust_env=False, headers=headers) as client:
            r = await client.get("https://api.crossref.org/works", params={"query.bibliographic": query, "rows": str(limit), "select": "DOI,title,URL,author,published-print,published-online,type,publisher"})
            r.raise_for_status(); body = r.json()
        out = []
        for item in body.get("message", {}).get("items", [])[:limit]:
            ...
            authors = [" ".join(str(a.get(k) or "") for k in ("given", "family")).strip() for a in (item.get("author") or [])[:8]]
            ...
            out.append(SpecialistResult(url=url, title=title, snippet="Authors: " + ", ".join(x for x in authors if x), engine="crossref", category="academic", published_at=published, raw={"doi": doi, "publisher": item.get("publisher"), "type": item.get("type")}))
        return out
    ...
    async def _arxiv(self, query: str, limit: int) -> list[SpecialistResult]:
        ...
        for entry in root.findall("a:entry", ns)[:limit]:
            ...
            out.append(SpecialistResult(url=link, title=title, snippet=summary, engine="arxiv", category="academic", published_at=published, raw={"arxiv_id": link.rsplit("/",1)[-1]}))
        return out
