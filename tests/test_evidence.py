...
async def test_claim_extractor_rejects_non_literal_quotes(tmp_path: Path):
    ...
    sid = storage.add_search_result(run, "q", 1, SearchResult(url="https://example.com", title="x"))
    source = storage.sources_for_run(run)[0].model_copy(update={"text_excerpt": "some exact evidence in source"})
    claims = await EvidenceAnalyzer(storage, FakeProvider()).extract_claims(run, ResearchSpec(topic="example"), source)
    assert len(claims) == 1
