...
def test_frontier_is_best_first_and_depth_bounded(tmp_path: Path):
    ...
    manager = FrontierManager(
        storage, SafeFetcher(timeout=1, max_bytes=1000, user_agent="test"), user_agent="test"
    )
    links = [
    ...
    assert (
        manager.add_page_links(
            run, spec, source_id=parent_id, parent_url="https://acme.example", links=links, depth=1
        )
        >= 1
    )
