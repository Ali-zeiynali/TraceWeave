    ...
    async def search(self, query: str, *, limit: int, language: str):
        ...
        return [
            SearchResult(
                ...
                title="Acme current",
                snippet="Acme acquired Beta in 2025",
                engine="fake",
                category="web",
            )
        ][:limit]
    ...
    async def json(self, *, system: str, user: str, task="general", run_id=None):
        ...
        if task == "claim_extraction":
            return {
                "claims": [
                    {
                        "claim": "Acme acquired Beta in 2025",
                        "subject": "Acme",
                        "predicate": "acquired",
                        "object": "Beta",
                        "observed_at": "2025",
                        "evidence_quote": "Acme acquired Beta in 2025",
                        "confidence": 0.92,
                    }
                ]
            }
        if task == "entity_extraction":
            ...
            return {
                "entities": [
                    ...
                    {"name": "Beta", "type": "organization", "confidence": 0.9},
                ],
                "relationships": [
                    {
                        "source": "Acme",
...
class FakeFetcher:
    async def fetch(self, url: str, max_redirects: int = 5, accept=None, max_bytes=None):
        ...
        return FetchResult(
            ...
            text=text,
            title="fixture",
            content_hash="hash-" + str(abs(hash(url))),
    ...
    async def captures(self, url: str, limit: int):
        return [
            ArchiveCapture(
                engine="wayback",
                original_url=url,
                capture_url="https://web.archive.org/web/20240101000000id_/https://example.org/current",
                captured_at="20240101000000",
                mime="text/html",
                status=200,
                digest="d1",
                raw={"fixture": True},
    ...
    async def captures(self, url: str, limit: int):
        return [
            ArchiveCapture(
                engine="commoncrawl",
                original_url=url,
                capture_url="ccwarc://fixture#0:50",
                captured_at="20230101000000",
                mime="text/html",
                status=200,
                digest="cc1",
                raw={"filename": "fixture", "offset": 0, "length": 50},
            )
        ][:limit]
...
async def main() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        settings = Settings(
            ...
            registry_sources_enabled=False,
            public_social_enabled=False,
        )
        ...
        engine = ResearchEngine(
            settings=settings, storage=storage, search=FakeSearch(), planner=Planner(None), provider=provider
        )
        engine.fetcher = FakeFetcher()
        engine.specialists.fetcher = engine.fetcher
        engine.specialists.academic = FakeAcademic()
        engine.specialists.github = FakeGitHub()
        engine.specialists.wayback = FakeWayback()
        engine.specialists.commoncrawl = FakeCommonCrawl()
        engine.specialists.open_web = FakeOpenWeb()
        spec = ResearchSpec(
            topic="Acme technology",
            angle="history and technical evidence",
            mode="deep",
            max_rounds=2,
            max_results_per_query=1,
            fetch_top_per_query=1,
            max_depth=1,
            max_frontier_pages=4,
        )
        ...
        assert not any(r["predicate"] == "invented" for r in storage.relationships_for_run(run_id)), (
            "ungrounded graph relation admitted"
        )
        # Archive source state should avoid repeating successful archive API work on the second round.
        current = next(
            s for s in storage.sources_for_run(run_id, 500) if s.url == "https://example.org/current"
        )
        assert storage.source_stage_state(run_id, current.id, "archive:wayback")["status"] == "done"
        print(
            "SMOKE_STAGE45_OK",
            run_id,
            len(storage.sources_for_run(run_id, 500)),
            len(storage.claims_for_run(run_id, 500)),
        )
