...
class FakeSearch:
    ...
    async def search(self, query: str, *, limit: int, language: str):
        ...
        return [
            SearchResult(
                url="https://example.com/root", title="Root report", snippet="battery evidence", engine="fake"
            )
        ]
    ...
    async def json(self, *, system: str, user: str, task: str = "general", run_id: str | None = None):
        ...
        if task in {"planning", "replanning"}:
            return {
                ...
                "focus": ["documents"],
                "queries": ["synthetic battery report"],
                "gaps": ["confirmation"],
                "source_classes": ["documents"],
                "rationale": "smoke",
            }
        if task == "triage":
            return {
                ...
                "rationale": "smoke",
                "topics": ["battery"],
                "leads": ["child document"],
            }
        if task == "claim_extraction":
            return {
                "claims": [
                    {
                        "claim": "The synthetic source contains evidence.",
...
class OfflineEngine(ResearchEngine):
    async def _fetch_source(self, run_id: str, spec: ResearchSpec, source_id: int, url: str, *, depth: int):
        ...
        links = (
            []
            if "child" in url
            else [PageLink("https://example.com/child", "child battery document", "citation")]
        )
        result = FetchResult(
            final_url=url,
            status_code=200,
        ...
        self.storage.save_snapshot(
            ...
            raw=raw,
            text=text,
            extracted_title="Synthetic",
            simhash=result.simhash,
        )
        if depth < spec.resolved_depth():
            self.frontier.add_page_links(
                run_id, spec, source_id=source_id, parent_url=url, links=links, depth=depth + 1
            )
        return result
...
async def main() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        run_id = await engine.start(
            ResearchSpec(
                topic="synthetic battery",
                mode="standard",
                max_rounds=2,
                max_depth=2,
                max_frontier_pages=2,
